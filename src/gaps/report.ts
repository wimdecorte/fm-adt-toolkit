...
const BIGGEST_GAPS: Array<{ title: string; op: string; where: (e: SubjectEntry, a: Attribute) => boolean; story: string }> = [
  { title: 'Layout parts', op: 'read:layout', where: (e) => e.id.startsWith('part:'),
    story: 'fm 0.7.0 reports the part list at last, as `parts` on the layout itself: each part’s type, height, offset from the top of the layout, name, and a sub-summary’s break field with its occurrence. What is still unread is the Part Definition dialog — the page-break and page-numbering options, "allow part to break across page boundaries", the alternate and active row state — which fm carries in no key at all, plus the numeric part-type code behind the type name and the count of objects sitting in the band.' },

...
const BIGGEST_GAPS: Array<{ title: string; op: string; where: (e: SubjectEntry, a: Attribute) => boolean; story: string }> = [
  { title: 'Layout parts', op: 'read:layout', where: (e) => e.id.startsWith('part:'),
    story: 'fm 0.7.0 reported the part list at last, as `parts` on the layout itself: each part’s type, height, offset from the top of the layout, name, and a sub-summary’s break field with its occurrence. fm 0.8.0 adds the Part Definition dialog, which no read reached before it: `pagination` carries the page-break and page-numbering options — break before each occurrence, break after every N, restart page numbers, allow the part to break across page boundaries, discard the remainder — and the body also reports `rowState`, the alternate and active row state. Which members appear depends on the part type: a header, footer or title header carries `restartPageNumbers` alone. What is still unread is the numeric part-type code behind the type name, the part definition kind, the count of objects sitting in the band, and `pagination` itself on the three part types fm omits it for — top navigation, title footer and bottom navigation.' },
  { title: 'File Options', op: 'read:fileOptions', where: (e) => e.id === 'file-options',
    story: 'fm 0.8.0 adds a catalog for this, and it closes most of the gap: `read:fileOptions` answers with the startup layout, the minimum FileMaker version allowed to open the file, the login mode, the stored-credential and sign-in settings, the file icon, the spelling and text options, container thumbnail handling, and every file script trigger with its event, event code and script. What it still does not report is the two "don’t display in host’s file list" flags — one for FileMaker clients, one for WebDirect — the page setup saved with the file (orientation, scale and paper size), and whether a file script trigger is enabled in Browse mode.' },
  { title: 'Themes', op: 'read:theme', where: (e) => e.id === 'theme',
    story: 'fm 0.7.0 has a theme catalog of its own: `read:theme` describe answers with the whole stylesheet as text, the five palette swatches, every named style’s key and display name, and the layouts using the theme. What it still does not say is where the theme came from and how FileMaker lays out on it — the base theme and base version a custom theme derives from, the locale, platform and theme version, the chart colour scheme, and the 25 layout-builder metrics the New Layout/Report assistant measures a new layout by.' },
  { title: 'Sort specifications', op: 'read:layout', where: (_e, a) => /sort/i.test(a.name),
    story: 'A relationship’s sort order is readable in 0.7.0 — `rightToLeft.sortSpec.fields` names each level’s field and direction — but neither of its two switches is. `maintain` ("keep records in sorted order") is a confirmed gap: the reference file sets it and fm reports no such key. `blanksLast` is reported by no read either, though this file cannot prove fm never reports it — both of its sort specifications set it to false. Both keys are named on fm’s own write surface. A portal was further behind and is not any more: fm 0.8.0 reports a portal’s `sort.fields` — each level’s field as `Occurrence::Field` and its direction — and its `filter` formula as text, where every earlier build said nothing about a portal’s sorting, not even that it sorted. What a portal still withholds is the same pair a relationship does, `maintain` and `blanksLast`, plus any id for the sort field: fm names it and reports no field or occurrence id to resolve it by.' },
  { title: 'Modification info', op: 'read:layout', where: (_e, a) => /modif/i.test(a.name),
    story: 'Almost every catalog element in the export carries who changed it, when, and how many times. fm reports the triple on `layout` and nowhere else — `modified.by`, `modified.account` and `modified.timestamp` — leaving the other 39 kinds in this register with none of it, under any op. Still the most widely repeated gap here, and the narrowest to close: the shape fm already returns on a layout would answer it everywhere.' },
  ...
  { title: 'Calculation tokens', op: 'none', where: (e) => e.id === 'calculation-tokens' || e.id === 'plugin-call-sites',
    story: 'fm 0.8.0 answers part of this at last. Sent the new `references: true`, `validate:calculation` returns the fields a formula refers to as {kind, name, occurrence, id} — the occurrence riding along with the field rather than being its own token — and its custom-function calls as {kind, name, id}. It is opt-in, and emitted only on the valid branch. Still named by nothing: variables, built-in functions, scripts, and plug-in functions. The plug-in case is the sharpest, because this build loads the installed plug-ins and so reports a formula calling one as valid while returning `references: []` for it — fm knows enough to accept the call and still will not say the call site is there. So "where is this field used" is answerable for fields and custom functions, and no further.' },
];
